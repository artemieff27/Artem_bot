...
bot.on("message", async (msg) => {
	...
	switch (state) {
		case STATES.START:
			bot.sendMessage(chatId, "Привет! Меня зовут Альбина, я занимаюсь ранним развитием детей в нашем уютном клубе «Маруся».
			Занятия в нашем клубе направлены на разные возрастные группы, они проходят в игровой форме и очень нравятся деткам. 
			Ребёнок учится быть самостоятельным, развивает координацию и моторику, мышление и логику, осваивает общие социальные навыки. Такие занятия полезны для адаптации в новой обстановке.
			А также в нашем клубе можно бесплатно пройти нейропсихологическую диагностику детям от 4 до 7 лет.Узнать подробнее и записаться можно в нашем боте или написав мне лично 
			", startReply);
			break;
